...
- **SP10**（demo 手动验收补丁，2026-09-23）：套件 runner 增 `initScripts` seam —— `runScenarioSuiteInBrowser` 逐 context `addInitScript`（先于 newPage），plugin 传入 `[COLLECTOR_SHIM_SCRIPT]`（legacy Ruling 7 同通道）。demo 验收实测发现套件页裸建 context 无 shim 通道 → 页内 analysis trace 无投递口、每步 drain 恒空、S6 归因列真实 run 恒 NULL（单测未拦截：suite-mode.test 以 globalThis 手动镜像 shim）。脚本体归 plugin，scenario 包不知 shim 内容（S4 依赖方向不破）；replay 路径不注入（S7：replay 不进 coverage 通道）。
